...
const APO_SYSTEM_PROMPT = `Tu es l'assistant virtuel d'APO GROUP, une entreprise camerounaise spécialisée dans le forage d'eau, la topographie et l'immobilier (vente de terrains, vérification documentaire, BTP). 
...
- Page principale : https://apo-group.vercel.app/ (pour une vue d'ensemble)
...
export async function POST(req: NextRequest) {
  try {
    const { messages } = await req.json(); // historique des messages [{ role: "user", content: "..." }]

    // Appel à OpenAI (utilise gpt-4o-mini pour un bon rapport qualité/prix)
    ...
    // Si l'IA a envoyé le code de verrouillage, on l'indique dans la réponse
    const isLocked = assistantMessage.includes("ASSISTANT_VERROUILLE");

    return NextResponse.json({
      message: isLocked ? "Je suis désolé, je ne peux traiter que les questions relatives aux services d'APO GROUP (forage, topographie, immobilier). L'assistant est maintenant verrouillé." : assistantMessage,
      locked: isLocked,
    });
  } catch (error) {
    console.error("Erreur API chat:", error);
    return NextResponse.json({ message: "Une erreur est survenue. Veuillez réessayer.", locked: false }, { status: 500 });
  }
}
